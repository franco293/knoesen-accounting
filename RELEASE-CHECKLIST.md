...
- **Cloudflare analytics follow-up reviewed:** `site.json` explicitly enables cookieless Cloudflare Web Analytics; the privacy notice discloses it in sections 3, 6 and 7 and explains that it sits outside the cookie toggles. Cloudflare's [collection documentation](https://developers.cloudflare.com/web-analytics/data-metrics/data-origin-and-collection/) identifies the observed beacon and its performance-measurement purpose; its [product description](https://developers.cloudflare.com/web-analytics/about/) states that Web Analytics does not collect or use visitors' personal data. The pre-consent script request alone is therefore not classified as a confirmed consent defect. Preserve this intended configuration. This technical finding is not a legal compliance determination; exact dashboard injection settings were not inspected. Google Analytics remains separately consent-gated.

- **SME content and performance follow-up:** source `9863e7a`, release `d92e55c8e807ebff`, deployed 22 September 2026 as Cloudflare version `096225df-c696-49a7-816a-d24d7b66ad62`. Four service introductions clarify the confirmed SME scope and link to enquiry preparation. Contact guidance adds scope, deadlines and sensitive-data precautions. Mobile homepage lower sections defer rendering; a three-run paired local test reduced median LCP from 3.44s to 2.80s, with no field-performance claim. Live Chromium checks passed six widths, section anchors, print fallback, disabled JavaScript, service links, cookie withdrawal, navigation and source-file matching. Prior production/rollback reference: `8631b888-a403-43c1-8491-f1ef845cefb8`. No credentials or testimonials were added; email work remains deferred. Private search analysis, enquiry tracking and legal/access review preparation are retained outside the public repository.
